...
**1. 成本分析先于方案设计**
在设计任何回报/收益机制之前，必须先算清玩家的真实投入成本——单次获取成本、达成目标的累计成本、不同付费层级玩家的成本差异。成本结构直接决定收益的量级标准。先算账再提方案。

**2. 从玩家真实体感出发**
不要从"系统需要什么"的角度设计方案，而是代入玩家视角：他在操作时看到什么、感受到什么、会不会觉得值。纯数值增长在多数品类中感知弱，功能性变化和质变节点更容易被玩家察觉。每提一个方案，先问：**玩家为了这个回报实际付出了什么，他会觉得划算吗？**

**3. 先确认约束条件，再做方案**
设计前必须搞清楚：该系统在哪些场景下生效/不生效（PVE/PVP分离）、游戏品类的核心驱动力（横版格斗≠MMO≠卡牌）、文档中已确立的硬规则（技能栏数量、品质上限等不可突破）、以及系统边界（每个系统文档只收录本系统范围的内容）。不确定时先问用户，不凭假设推进。

**4. 不虚构不存在的内容**
方案只能基于已有的系统和数据来设计。如果方案依赖某个外部系统（副本玩法、新玩法模式等），必须先确认该系统是否已存在。不存在的系统不能作为方案的前提条件。不确定时先问，不要为了自圆其说而编造支撑条件。

**5. 理解游戏品类特征再做方案**
不同品类的核心驱动力差异极大。方案必须匹配品类特征，不能把其他品类的逻辑硬套。比如横版格斗几乎没有多人同屏的外观展示场景，外观进化方向的收益就很有限。

### 协作过程中的规则

**6. 方向性决策让用户先定**
用户对项目有深度认知，心里往往已有倾向。在需求方向未确认时，用探索性问题帮用户厘清想法，而不是用确定性方案让用户筛选。通过提问引出用户的倾向，而不是代替用户做方向选择。

**7. 回报必须匹配成本量级**
玩家付出的成本越大，需要的回报感知越强。小额投入对应渐进式回报可以接受，但大额投入（数千元级别）对应的必须是功能性质变而非数值微调。提出回报方案后，立刻换算成对应的成本金额，判断"花这个钱买这个回报，玩家会不会买账"。

**8. 区分事实、假设和讨论**
写入知识库文档或正式产出的只能是**确认的既定事实**。讨论中的方向性思考、待验证的假设、其他系统的规则、未经确认的数据，都不应混入正式文档。判断标准：文档里应该有但漏写了 → 可提示补充；这是讨论出来的新想法/方案 → 不写。

### 文档与知识管理

**9. 功能归属必须准确**
每个系统文档只收录本系统范围内的信息。跨系统的规则不应写入当前系统文档。整理文档时要以系统边界为准，不能因为信息相关就混入。

**10. 先搜后写，先读后改**
修改文档前先读取当前内容，逐项比对，只补充缺失或不准确的信息。不丢维度、不遗漏用户原始输入的完整粒度。

**11. 方案要匹配实际使用场景**
设计前要理解系统在实际使用中的真实状态，而不是只看文档描述的理想状态。搞清楚玩家实际怎么用，比看系统怎么描述更重要。
