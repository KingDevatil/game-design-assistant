...
1. **索引优先加载**：Skill 启动时从 `{project_knowledge_path}` 递归加载各子目录下的 `INDEX.md` 索引文件，建立文件清单，**不会一次性加载所有知识文件的完整内容**
2. **按需切片**：根据用户 query 关键词匹配到对应文件后，按需切片加载具体段落（非完整文件）
3. **优先级**：当项目知识库与通用知识库冲突时，**以项目知识库为准**
4. **更新机制**：项目设计规则变更时，同步更新本项目知识包中的对应文件。新增文件时需在所在目录的 `INDEX.md` 中添加条目
5. **目录结构**：可根据需要自由组织二级目录，每个目录需有 `INDEX.md` 说明文件清单
6. **禁止假设**：Skill 在使用项目知识库时，不得擅自增删或修改已定义的规则
